...
* Copying files requires that the NFS share allows access from the Data Sense instance.
...
* You must have the Account Admin or Workspace Admin role to copy files.
* The volume must be from a Cloud Volumes ONTAP or on-premises ONTAP system. No other data sources are supported.
* All selected files must be from the same volume.
