...
* You can move or delete files that seem insecure or too risky to leave in your storage system, or that you have identified as a duplicate.
...
+
This is useful for situations where you're migrating data but you want to exclude certain files from the original volume. Or you might want to create a copy of a volume for testing, or to provide access to the volume for additional users without giving them access to the production data. This action uses the NetApp FlexClone functionality to quickly duplicate the volume and then remove the files that you did not select.
...
You can clone an existing ONTAP volume that Data Sense is scanning using NetApp _FlexClone_ functionality. This allows you to quickly duplicate the volume while including only those files you selected. This is useful if you're migrating data but you want to exclude certain files from the original volume, or if you want to create a copy of a volume for testing.
...
. In the _Copy Files_ dialog, select the *FlexClone* tab. This page shows the total number of files that will be cloned from the volume (the files you selected), and the number of files that are not included (deleted) from the cloned volume.
...
The new volume is created in the same aggregate as the source volume. If you initially selected *Activate scan for all volumes* when you enabled Data Sense for the working environment where the source volume resides, then Data Sense will scan the new cloned volume automatically. If you didn't use this selection initially, then if you want to scan this new volume, you'll need to link:task_getting_started_compliance.html#enabling-and-disabling-compliance-scans-on-volumes[enable scanning on the volume manually].
