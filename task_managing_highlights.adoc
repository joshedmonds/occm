...
When you run an action from the Investigation Results pane across many files, for example, deleting 50 files, the process can take some time. You can monitor the status of these asynchronous actions so you'll know when it has been applied to all files. This allows you to see the actions that have completed successfully, those currently in progress, and those that have failed so you can diagnose and fix any problems.
...
Note that you can Cancel any actions that have the "Queued" and "In Progress" status.

.Steps

. In the bottom-right of the Configuration tab you can see the button *Action Status*.
...
[NOTE]
====
* You can't currently change labels in files larger than 30 MB. For OneDrive accounts the maximum file size is 4 MB.
