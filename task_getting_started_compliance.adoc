...
If you don't want Cloud Data Sense to scan volumes directly on your on-prem ONTAP systems, a Beta feature released in January 2021 allows you to run compliance scans on backup files created from your on-prem ONTAP volumes. So if you're already creating backup files using link:concept_backup_to_cloud.html[Cloud Backup^], you can use this feature to run compliance scans on those backup files.

The Compliance scans you run on backup files are *free* - no Cloud Data Sense subscription or license is needed.

[NOTE]
====
* When Data Sense scans backup files it uses permissions granted through the Cloud Restore instance to access the backup files. Typically the Restore instance powers down when not actively restoring files, but it remains *On* when scanning backup files. See link:task_restore_backups.html#file-restore-process[more information about the Restore instance^].
* Since there is no current support for restoring single files when using GCP, Cloud Data Sense can't scan backup files in GCP.
====
