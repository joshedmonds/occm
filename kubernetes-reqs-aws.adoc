...
Astra Trident::
The EKS cluster must have NetApp Astra Trident installed. One of the four most recent versions of Astra Trident is required. https://docs.netapp.com/us-en/trident/trident-get-started/kubernetes-deploy-operator.html[Go to the Astra Trident docs for installation steps^].
...
Network connectivity is required between the EKS cluster and the Connector and between the EKS cluster and Cloud Volumes ONTAP. <<Review networking requirements,Learn more below>>.

RBAC authorization::
The Cloud Manager Connector role must be authorized on each EKS cluster. <<Set up RBAC authorization,Learn more below>>.
