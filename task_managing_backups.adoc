...
Note that the following tasks are performed the same way for ONTAP clusters (Cloud Volumes ONTAP and on-premises ONTAP) and for Kubernetes clusters, but the text on the UI pages are slightly different. ONTAP cluster UI pages are shown in the examples.
...
. From the _Backup Settings page_, click image:screenshot_horizontal_more_button.gif[More icon] for the working environment, or the Kubernetes cluster, where you want to change the settings and select *Modify Policy*.
+
...
+
You can choose to have all volumes added in the future to have backup enabled, or not, by using the checkbox for "Automatically back up future volumes...". If you disable this setting, you'll need to manually enable backups for future volumes.
