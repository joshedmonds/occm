...
Changing the backup policy affects both new volumes created after you change the schedule, and any existing volumes that were using the original policy. Note that any backup files that have been tiered to archival storage are left in that tier if you stop tiering backups to archive - they are not moved back to the standard tier.
...
+
If your cluster is running ONTAP 9.10.1 or greater, and you are using AWS or Azure for your cloud storage, you will also have the option to tier backups to archival storage after a certain number of days.
link:reference-azure-backup-tiers.html[Learn more about using Azure Archive storage].
link:reference-aws-backup-tiers.html[Learn more about using S3 Glacier or S3 Glacier Deep Archive storage].
+
