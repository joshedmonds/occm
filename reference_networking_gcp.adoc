...
=== Using your own firewall rule for VPCs 1-3

When you create an HA pair, Cloud Manager gives you the option to use predefined firewall rules or to use your own. If you use your own rules and you're deploying the HA pair across multiple Google Cloud zones, then you must set up a target tag for the firewall rule and for Cloud Volumes ONTAP.

. Choose a value for the tag (it can be any value that you'd like).
. When you create the Cloud Volumes ONTAP HA pair in Cloud Manager, specify that tag on the Details and Credentials page in the *Add Labels* field.
. When you create the firewall rule in Google Cloud, go to the *Targets* field, select *Specified target tags*, and enter the tag.
+
The value of the tag must be the same for steps 2 and 3.
