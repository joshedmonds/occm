...
When you create an HA pair, Cloud Manager gives you the option to use predefined firewall rules or to use existing rules. If you use your own firewall rules for these VPCs and you're deploying the HA pair across multiple Google Cloud zones, then you must set up a _target tag_ for the firewall rule and then use that same value as a _label_ on the HA pair.

. Choose a value for the firewall target tag and the HA pair label.
+
The value can be any text string that you'd like, but it must be exactly the same for the target tag and the HA pair label.
...
. When you create the Cloud Volumes ONTAP HA pair in Cloud Manager, specify the value on the Details and Credentials page in the *Add Labels* field.
+
