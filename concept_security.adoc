...
Setting up a supported key manager is the only required step. For set up instructions, refer to link:task_encrypting_volumes.html[Encrypting volumes with NetApp encryption solutions].
...
You can use your own encryption keys if you prefer. link:task_set_up_azure_encryption.html[Learn how to set up Cloud Volumes ONTAP to use a customer-managed key in Azure].
