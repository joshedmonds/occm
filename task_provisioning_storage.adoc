...
* Specify details for a new volume and let Cloud Manager handle the underlying data aggregates for you. <<Create a volume on any aggregate,Learn more>>.
* Create volume from a template to optimize the volume for the workload requirements for certain applications, such as databases or streaming services. <<Create volumes from templates,Learn more>>.
* Create a volume on a data aggregate of your choice. <<Create a volume on a specific aggregate,Learn more>>.
...
Click *Create volume*, specify your storage needs, and let Cloud Manager handle the disk allocation for you.
...
+
Details about the fields on this page are provided by protocol.
+
// start tabbed area
[role="tabbed-block"]
====
...
.NFS
--
A custom NFS export policy defines the clients in the subnet that can access the volume. By default, Cloud Manager enters a value that provides access to all instances in the subnet.
--

.CIFS
--
The *Permissions* and *Users/Groups* fields enable you to control the level of access to an SMB share for users and groups (also called access control lists or ACLs). You can specify local or domain Windows users or groups, or UNIX users or groups. If you specify a domain Windows user name, you must include the user’s domain using the format domain\username.

If a CIFS server has not been set up, Cloud Manager prompts you to set up CIFS connectivity after you click *Next*.
+
...
If you have a good understanding of the data aggregates on your Cloud Volumes ONTAP system, you can choose a specific aggregate for a new volume.

.Steps

. On the Canvas page, double-click the name of the Cloud Volumes ONTAP system on which you want to provision FlexVol volumes.

. Select the option to create a volume on a specific aggregate:
+
...
. On the Usage Profile, Disk Type, and Tiering Policy page, edit the volume tiering policy, if needed.
+
For help with tiering policies, refer to link:concept_data_tiering.html[Data tiering overview].
