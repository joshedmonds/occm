...
+
The following tables describe fields for which you might need guidance.
+
[role="tabbed-block"]
====
...
| Operating system type | Specifies the operating system for the initiator group.
...
.. *Protocol*: Choose a protocol for the volume (NFS, CIFS, or iSCSI) and then provide the required information.
+
The following sections describe fields for which you might need guidance.
+
[role="tabbed-block"]
====
...
If a CIFS server has not been set up, Cloud Manager prompts you to set up CIFS connectivity after you click *Next*. The fields on to set up CIFS are described below.
