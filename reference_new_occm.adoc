...
* Cloud Sync is now integrated with Cloud Data Sense. You can use Data Sense to copy files from a single source repository (ONTAP volume, S3 bucket, NFS share, etc.) to a target location and synchronize those files.
+
link:task_managing_highlights.html#copying-and-synchronizing-source-files-to-a-target-system[Learn how to copy and sync files].
