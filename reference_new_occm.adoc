...
When you create a new Connector, Cloud Manager will now display all of your existing firewall policies. Previously, Cloud Manager wouldn't display any policies that didn't have a target tag.

=== Credentials enhancement

We refined how you add AWS credentials to Cloud Manager. You can now add the credentials to an existing Connector or to the Cloud Manager SaaS:

* Add AWS credentials to an existing Connector
+
Adding AWS credentials to an existing Connector enables you to deploy Cloud Volumes ONTAP in another AWS account using the same Connector. You can specify an IAM role for the Connector to assume or provide AWS access keys.

* Add AWS credentials directly to Cloud Manager
+
Adding the ARN of an IAM role to Cloud Manager enables Cloud Manager to assume that role when creating an FSx for ONTAP working environment.
