...
== Cloud Manager 3.9.11 (3 Oct 2021)
...
+
link:task_manage_fsx_volumes.html#creating-volumes[Create FSx for ONTAP CIFS volumes using Cloud Manager]

* Now you can edit FSx for ONTAP volumes using Cloud Manager.
+
link:task_manage_fsx_volumes.html#editing-volumes[Edit FSx for ONTAP volumes using Cloud Manager]
...
* Data Sense can now scan FSx for ONTAP CIFS and Data Protection (DP) volumes. link:task_scanning_fsx.html[Learn how how to scan Amazon FSx for ONTAP volumes].

* Now you can choose to scan or map individual volumes in your working environments and data sources.
+
In the past you could either map or scan all volumes in each data source. Now you can choose to map _or_ scan individual volumes in each data source. This option is available for Cloud Volumes ONTAP volumes, ANF volumes, on-prem ONTAP volumes, FSx for ONTAP volumes, and volume backups.

* Support for BYOL licensing from NetApp.
+
In addition to subscribing to use Data Sense through your cloud provider marketplace, now you can purchase a bring-your-own-license (BYOL) from NetApp that you can use across all your working environments and data sources in your account.
+
link:task-licensing-cloud-data-sense.html#use-a-cloud-tiering-byol-license[Learn more about the new Cloud Data Sense BYOL license].
