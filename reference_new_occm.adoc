...
You can now disable S3 accelerated downloads of software updates for Cloud Volumes ONTAP in Google Cloud, which can resolve an issue where the Connector failed to download software updates when using a NAT Gateway in Google Cloud.

Learn how to disable S3 accelerated downloads in Google Cloud (link coming soon).
...
* Now you can choose to scan or classify individual volumes in your working environments.
+
...
* You can copy any source files that Data Sense is scanning directly from Data Sense. There are two types of copy operations:
+
** Copy files from the same, or different, volumes to a destination NFS share. This is helpful if you want to make a copy of certain data and move it to a different NFS location.
** Clone an ONTAP volume, but include only selected files from the source volume in the new cloned volume. This is helpful for situations where you’re migrating data and you want to exclude certain files, or if you want to create a copy of a volume for testing.

* Existing functionality to delete a single file or add tags to a single file is now expanded so you can delete multiple files at a time and apply the same tags to multiple files at a time.
...
+
Learn how to authenticate with an Azure service principal (link coming soon).

*	You can now install the Connector in an on-premises site that doesn't have internet access, and then discover on-prem ONTAP clusters and scan them with Cloud Data Sense. Learn more in the Cloud Data Sense section below (link coming soon).
+
Note that the installation must be completed by using a new on-prem Connector installer that's specifically for sites that don't have internet.
