...
* See <<Verify Cloud Manager and Data Sense prerequisites,Reviewing prerequisites>> for the full list of requirements and endpoints that Cloud Data Sense must be able to reach over the internet.

* The host requirements are the same for Scanner nodes as they are for Manager nodes. See <<Prepare the Linux host system,Host requirements>> for details.
