...
* An on-prem ONTAP cluster that's running ONTAP 9.2 or later and has an HTTPS connection to Amazon S3. link:task_discovering_ontap.html[Learn how to discover a cluster].
...
The S3 bucket must be in a link:reference_aws_support.html#supported-aws-regions[region that supports Cloud Tiering].
