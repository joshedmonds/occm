...
= Deploy Cloud Data Sense without internet access
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
When installed in this manner (some call this a "dark site"), Data Sense can only scan data from data sources that are also local to the on-premises site. At this time, Data Sense can scan the following local data sources:
...
If you don't already have a Connector installed at your on-premises site, link:task_installing_linux.html[deploy the Connector^] on a Linux host in the dark site.
...
If you don't already have a Connector, link:task_installing_linux.html[deploy the Connector^] on an existing Linux host in your dark site.
