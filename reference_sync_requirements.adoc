...
. When you create a sync relationship from Amazon S3 to Box, you must use a data broker group that has a unified configuration where the following settings are set to 1:
+
* Scanner Concurrency
...
+
link:task_sync_managing_data_brokers.html#define-a-unified-configuration-for-a-data-broker-group[Learn how to define a unified configuration for a data broker group].
...
Cloud Sync supports https://www.box.com/home[Box^] in a sync relationship when using the Cloud Sync API.
