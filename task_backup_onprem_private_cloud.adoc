...
[role="quick-margin-para"]
image:screenshot_backup_from_onprem_activate.png["A screenshot that shows the Backup & Restore Enable button which is available after you select an on-prem working environment."]
...
. From the Canvas, select the on-premises working environment and click *Enable* next to the Backup service in the right-panel.
+
image:screenshot_backup_from_onprem_activate.png[A screenshot that shows the Backup Enable button which is available after you select an on-prem working environment.]
