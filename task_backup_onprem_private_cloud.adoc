...
When the Connector is installed in an on-prem location without internet access, the StorageGRID system must be located in the same on-prem data center.
...
* The nodes' intercluster LIFs are able to access the internet (not required when the Connector is installed in a "dark" site).
...
When backing up data to StorageGRID, a Connector must be available on your premises. The Connector can be installed in a site with or without internet access. You'll either need to install a new Connector or make sure that the currently selected Connector resides on-prem.
