...
There's one object store per Cloud Volumes ONTAP or on-premises ONTAP system. Cloud Manager names the object store as follows: "netapp-backup-_clusteruuid_". Be sure not to delete this object store. Currently the relationship between a source cluster and a destination backup store is 1:1. If you wish to change the destination object store, you would have to go through the process of https://docs.netapp.com/us-en/occm/task_managing_backups.html#disabling-cloud-backup-for-a-working-environment[Disabling and Unregistering backups for that source cluster^].
