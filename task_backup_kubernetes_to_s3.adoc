...
summary: Complete a few steps to get started backing up data from Kubernetes persistent volumes to Amazon S3.
---
...
[.lead]
Complete a few steps to get started backing up data from Kubernetes persistent volumes to Amazon S3.
...
[role="quick-margin-para"]
Restore an entire backup to a new volume on the same or different Cloud Volumes ONTAP system in AWS.
