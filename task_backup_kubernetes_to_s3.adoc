...
Additionally, the Cloud Manger role has to have the following permissions:
+
[source,json]
eks:DescribeCluster
eks:ListClusters
...
An S3 bucket is created automatically in the same AWS account and Region as the Cloud Volumes ONTAP system, and the backup files are stored there. The Volume Backup Dashboard is displayed so you can monitor the state of the backups.
