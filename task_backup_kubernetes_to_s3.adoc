...
* All Persistent Volume Claim objects that will be used to create the persistent volumes that you want to back up must have "snapshotPolicy" set to "default". This can be done using `snapshotPolicy` annotation on the PVC, or using the `snapshotPolicy` field under defaults in the `backend.json` file. See the bolded text in the following examples:
+
[source,json]
