...
If Cloud Manager fails to create an Azure Private Link as part of specific actions, it completes the action without the Azure Private Link connection. This can happen when creating a new working environment, creating a new aggregate, adding capacity to an existing aggregate, or creating a new storage account when going above 32 TiB.
