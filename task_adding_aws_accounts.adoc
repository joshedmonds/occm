...
Add AWS credentials to enable the Connector to deploy and manage Cloud Volumes ONTAP in other AWS accounts. You can either provide the ARN of an IAM role in a trusted account or provide AWS access keys.
...
. Enable the Connector instance to assume the IAM role:
...
The account now has the required permissions. <<add-the-credentials,You can now add the credentials to Cloud Manager>>.
...
The account now has the required permissions. <<add-the-credentials-2,You can now add the credentials to Cloud Manager>>.
...
Create an IAM role that Cloud Manager can assume in order to create a new Connector, or to create an FSx for ONTAP working environment.
