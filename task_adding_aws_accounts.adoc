...
Choose Connector to add a new AWS account to an existing Connector. This enables you to deploy and manage Cloud Volumes ONTAP in other AWS accounts from the same Connector. Choose Cloud Manager to add new AWS credentials to Cloud Manager. This enables you to create a new Connector or to use the credentials with a cloud service like FSx for ONTAP.

Add credentials to enable the Connector to manage resources in other AWS accounts. You can either provide AWS access keys or provide the ARN of an IAM role in a trusted account.


Cloud Manager

Provide the ARN of an IAM role that gives Cloud Manager the permissions needed to create and manage FSx for ONTAP or to create a new Connector.

Assume Role

Provide the ARN of an IAM role that gives Cloud Manager the permissions needed to create and manage FSx for ONTAP or to create a new Connector.

Learn how to create the role
1
Go to the IAM console.
2
Under Access Management, click Roles > Create Role.
3
Select Another AWS account and enter the Cloud Manager account ID: 733004784675
4
On the Attach permissions policies page, create a policy using the required permissions, and then complete the remaining steps.
5
Copy the Role ARN and paste it on this screen.


Connector

Assume role or AWS keys

Assume Role
Enable the Connector to manage additional AWS accounts by providing an IAM role in another account that the Connector can assume.
...
AWS Keys
An access key and secret key for an AWS account in which Cloud Manager can launch Cloud Volumes ONTAP systems.
...
. Go to the target account where you want to deploy Cloud Volumes ONTAP and create an IAM role by selecting *AWS account*.
+
