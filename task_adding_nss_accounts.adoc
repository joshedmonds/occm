...
. At the login page, provide your NetApp Support Site registered email address and password to perform the authentication process.
+
After the process is complete, the account that you updated should now be listed as a new account in the table. The older version is still listed in the table, along with any existing working environment associations.
...
. For the working environment that you want to change the association for, click image:icon-action.png["An icon that is three side-by-side dots"] and then select *Change to a different NSS account*.
+
