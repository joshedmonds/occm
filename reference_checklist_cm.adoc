...
a| In most cases, you should work with Cloud Manager from the SaaS UI, but if you need to use the link:concept_connectors.html#using-multiple-connectors-with-the-same-working-environment[local UI], then you must enter the host’s IP address from a web browser.
