...
The Cloud Manager user interface is accessible from a web browser. As you use the Cloud Manager user interface, it contacts several endpoints to complete data management tasks.
+
The machine running the web browser must have connections to the following endpoints.
+
...
* If you choose to manually install the Connector on your own Linux host (and not do so directly from the Cloud Manager interface), then the installer for the Connector must access the following URLs during the installation process:
+
