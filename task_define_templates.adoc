...
== Examples of finding existing resources using templates so you can activate cloud services

Using the _Find Existing Resources_ action you can find existing resources and enable a cloud service on those resources.

At this time you can find volumes within Cloud Volumes ONTAP, on-premises ONTAP, and Azure NetApp Files systems only. And you can enable only Cloud Backup on these existing volumes. Additional resources and service will be available at a later time.

=== Find existing volumes so you can activate cloud backup
...
. *Policy*: Define the backup policy to create daily, weekly, or monthly backups, and specify the number of backup copies to retain.
