...
== Disable S3 accelerated downloads in Google Cloud

By default, the Connector downloads software updates for Cloud Volumes ONTAP using S3 accelerated downloads. The download can fail if your configuration uses a Google Cloud NAT gateway. The failure occurs because the download opens multiple concurrent sessions that exceed the number of available ports.

Disabling S3 accelerated downloads can resolve the issue. These steps must be completed by using the Cloud Manager API.
...
[source,json]
{
  "useAccelerationForImageDownload": false
}
