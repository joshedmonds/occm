...
* Add custom tags to files (for example, "needs to be moved") and assign a Cloud Manager user so that person can own the change to the files
...
NOTE: A Beta feature released in January 2021 allows you to run compliance scans on the backup files from your on-prem ONTAP volumes (created using link:concept_backup_to_cloud.html[Cloud Backup^]). This gives you the option to have Cloud Data Sense scan your on-prem ONTAP volumes directly, or scan the backup files made from those volumes.
...
| Create link:task_managing_highlights.html#controlling-your-data-using-policies[policies] that provide custom search results | Yes | No
