...
Center panel::
Shows all of your data sources. Healthy environments have a green indicator and environments that have alerts have a red indicator.
Right panel::
After you click on a data source that has a red indicator, this panel shows the alerts for that data source and provides recommendations to resolve the alert. Alerts are sorted so that the most recent alerts are listed first. Most recommendations lead you to another Cloud Manager service where you can resolve the issue.
...
| An abnormal increase in the percentage of encrypted files in the data source has occurred. This means that there was a greater than 20% increase in the number of encrypted files in the past 7 days.
| Click the link to launch the link:task_controlling_private_data.html#filtering-data-in-the-data-investigation-page[Data Sense Investigation page]. There you can select the filters for the specific _Working Environment_ and _Category (Encrypted)_ to view the list of all encrypted files.
...
| Data Sense is not scanning your working environment
| Data in the working environment isn't being scanned using link:concept_cloud_compliance.html[Cloud Data Sense] to identify compliance and privacy concerns, and find optimization opportunities.
| Click the link to launch Data Sense and enable scanning and mapping for the working environment.
...
The _Encrypted Files_ area shows the percentage of files that are encrypted in the top 4 data sources, over time. These are typically items that have been password protected. It does this by comparing the encryption rates over the past 7 days to see which data sources have a greater than 20% increase. An increase of this amount could mean that ransomware is already attacked your system.
