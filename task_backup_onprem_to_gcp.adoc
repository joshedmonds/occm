...
.image:https://raw.githubusercontent.com/NetAppDocs/common/main/media/number-6.png[Six] Activate Compliance scans on the backed up volumes (optional)
...
+
image:screenshot_backup_from_onprem_activate.png[A screenshot that shows the Enable button which is available after you select an on-prem working environment.]
