...
== Setting up permissions by creating service accounts
...
.From gcloud
--
. Login to the gcloud SDK using your preferred methodology. I am using a local shell with the gcloud SDK installed, but you could use the native Google Cloud Shell in the GCP console. For more information about the Google Cloud SDK, visit the link:https://cloud.google.com/sdk[Google Cloud SDK documentation page].

. Check that you are logged in as a user who has the correct permissions defined above with the command:
+
[source,bash]
gcloud auth list
...
. Run the following "compute create" command:
+
[source,bash]
gcloud compute instances create <<instance_name>> \ // <1>
  --machine-type=n1-standard-4 \
  --image-project=netapp-cloudmanager \
  --image-family=cloudmanager \
  --scopes=cloud-platform \
  --project=<<project>> \ // <2>
  --service-account=<<connectorServiceAccount@project.iam.gserviceaccount.com>> \ // <3>
  --zone=<<zone>> \ // <4>
  --no-address \ // <5>
  --tags <<network-tag>> \ // <6>
  --network <<network-path>> \ // <7>
  --subnet <<subnet-path>> \ // <8>
  --boot-disk-kms-key <<kms-key-path>> \ // <9>

+
<1> Desired instance name
<2> (Optional) Project to deploy VM into
<3> Service account specified in Create Service Accounts step 2
<4> Desired zone for the VM
<5> (Optional) No external IP address is used (Need a cloud NAT or proxy to route traffic to public internet)
<6> (Optional) Add network tagging to link a firewall rule using tags to the connector instance
<7> (Optional) Add the name of the network to deploy the service connector into (For Shared VPC you need the full path)
<8> (Optional) Add the subnet name to deploy the service connector into (For Shared VPC you need the full path)
<9> (Optional) Add a kms key to encrypt the service connector disks (IAM permissions also need to be applied)
+
...
. Run the command and GCP will deploy the connector using the NetApp golden image. The Connector instance and software should be running in approximately five minutes.
